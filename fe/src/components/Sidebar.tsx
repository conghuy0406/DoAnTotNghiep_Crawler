...
const Sidebar: React.FC<SidebarProps> = ({ activePage }) => {
  ...
  // Tự động mở menu Chức Năng nếu đường dẫn chứa từ 'crawler' hoặc 'schedule'
  const [isOverviewOpen, setIsOverviewOpen] = useState(
    location.pathname.includes('crawler') || location.pathname.includes('schedule')
  );

  // Cập nhật trạng thái mở khi chuyển trang bên ngoài sidebar
  useEffect(() => {
    if (location.pathname.includes('crawler') || location.pathname.includes('schedule')) {
  ...
  const menuItems = [
    { 
      name: 'Trang Chủ', path: '/home', icon: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' 
    },
    { 
  ...
  return (
    <aside className="fixed inset-y-0 left-0 w-20 md:w-64 bg-[#0b0724] border-r border-white/5 flex flex-col items-center md:items-start p-4 md:p-6 transition-all h-screen shadow-2xl z-50">
      ...
      <nav className="flex-1 w-full space-y-2 overflow-y-auto pr-2 custom-sidebar-scroll">
        {menuItems.map((item) => {
          // Kiểm tra xem trang con của item này có đang active không
